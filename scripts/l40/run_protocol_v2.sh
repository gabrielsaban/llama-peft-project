...
usage() {
  cat <<'EOF'
usage: scripts/l40/run_protocol_v2.sh [baseline|phase1|phase2|all]

modes:
  baseline  run baseline-only evaluations for LoRA and QLoRA phase-1 configs
  phase1    run core comparison training runs (LoRA r16, QLoRA r16)
  phase2    run QLoRA rank sweep training runs (r32, r64)
